...
\begin{enumerate}
  \item 后端配置多用户表使用Spring Security\\
        ...
        \begin{itemize}
          ...
          \item \lstinline[language = Java]| UsernamePasswordAuthenticationToken |：\lstinline[language = Java]| Authentication |的实现类，用于登录验证，最为常用。
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|l|l|}
    ...
    用户登录     & 验证不同用户是否能正确登录                 & 功能准确实现 \\ \hline
