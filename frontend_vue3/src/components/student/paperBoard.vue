...
<template>
  ...
  <div class="card-wrapper">
    <div class="card" v-for="(file, index) in tableData" :key="index">
      ...
      <div class="card-abstract-conten-wrapper">
        <textarea
          class="card-abstract-content"
          name="text"
          rows="14"
          cols="10"
          wrap="soft"
          v-model="file.fileAbstract"
        ></textarea>
      </div>
      ...
      <div class="card-footer">
        ...
        <el-button
          ...
          size="small"
          @click="openDialogComment(file.id)"
        ></el-button>
        <el-button
          class="glass-btn-important card-btn"
          icon="el-icon-delete-solid"
          size="small"
          @click="openDialogDelete(file.id)"
        ></el-button>
        <input
          type="file"
          class="refresh-btn-input-file"
          value
          :id="file.id"
          @change="handleFileRefresh($event, file.id)"
        />
  ...
  <!-- 删除文件 Dialog begin -->
  <teleport to="body">
    <el-dialog title="警告" v-model="dialogVisibleDelete" width="30%" :before-close="handleClose">
...
export default {
  ...
  setup(props) {
    ...
    const getComments = () => {
      store
        .dispatch('student/studentGetAllComment', {
    ...
    const handleCommentAdd = () => {
      store
        .dispatch('student/studentAddCommentForThisFile', {
          chosedFileId: chosedFileId.value,
